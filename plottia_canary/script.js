...
function joinRoom() {
    ...
    if (currentFileId !== fileIdFromUrl) {
         if (fileIdFromUrl) {
            ...
            console.warn("No fileId in URL hash.");
        }
        return;
    }
    ...
    switch (op.type) {
        ...
        case 'CREATE_SHAPE':
            ...
            if (boardData[collectionName] && !boardData[collectionName].some(item => item.id === op.payload.id)) {
                ...
                const createFnName = `create${op.type.split('_')[1].charAt(0).toUpperCase()}${op.type.split('_')[1].slice(1).toLowerCase()}`;
...
function undo() {
    ...
    // This is tricky with collaborative undo. A simple inverse might not be enough.
    // For now, this is a basic implementation.
    myUndoStack.pop(); // Remove the action that was just undone
}
...
function redo() {
    ...
    myUndoStack.pop(); // Remove the action that was just redone
}
...
function addCommonEventListeners(element, data) {
    ...
    header.addEventListener('mousedown', e => {
        if (element.classList.contains('locked') || e.target.closest('.resizer, [contenteditable="true"], .color-picker, .delete-btn, .lock-btn')) return;
        e.stopPropagation();
        
        const startZIndex = boardData.board.noteZIndexCounter++;
        let lastPos = getEventCoordinates(e);

        // --- Start of Section Drag Logic ---
        let attachedElements = [];
        if (element.classList.contains('section')) {
            const sectionLeft = parseFloat(element.style.left);
            const sectionTop = parseFloat(element.style.top);
            const sectionWidth = element.offsetWidth;
            const sectionHeight = element.offsetHeight;
            
            const allDraggables = [...boardData.notes, ...boardData.shapes, ...boardData.textBoxes];
            allDraggables.forEach(itemData => {
                const el = document.getElementById(itemData.id);
                if (!el) return;

                const elLeft = parseFloat(itemData.x);
                const elTop = parseFloat(itemData.y);

                if (elLeft >= sectionLeft && elLeft + el.offsetWidth <= sectionLeft + sectionWidth &&
                    elTop >= sectionTop && elTop + el.offsetHeight <= sectionTop + sectionHeight) {
                    attachedElements.push({
                        element: el,
                        offsetX: elLeft - sectionLeft,
                        offsetY: elTop - sectionTop
                    });
                }
            });
        }
        // --- End of Section Drag Logic ---
        ...
        const onPointerMove = ev => {
            ...
            // --- Move attached elements visually ---
            const newSectionLeft = parseFloat(element.style.left);
            const newSectionTop = parseFloat(element.style.top);
            attachedElements.forEach(item => {
                item.element.style.left = `${newSectionLeft + item.offsetX}px`;
                item.element.style.top = `${newSectionTop + item.offsetY}px`;
                item.element.style.zIndex = startZIndex + 1;
            });
        ...
        const onPointerUp = () => {
            ...
            // --- Build payload for all moved elements ---
    ...
    // Other event listeners (delete, lock, color, content)
...
function redrawCanvas() {
    if (!boardData || !boardData.paths) return;
...
async function initializeApp() {
    ...
    // Load saved stroke width on startup
